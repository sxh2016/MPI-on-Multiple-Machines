# Bonus for running mpi on multiple machines

## Prepare the machines

**start two virtual machines on my computer**

Start two fedora virtual machines on my local machine, one is vm1, another is vm2. Assign the vms in the same host-only network. 


Install mpich on the two machines.

**make keyless-ssh from vm1 to vm2**
...
Run the mpi job using the following command:
```
mpiexec -n 10 -hosts vm1,vm2 ./ring 10
```
...
```
[mpiuser@localhost mpi-ring-distsys15_hw6_ke_wang]$ mpiexec -n 10 -hosts 192.168.56.101,192.168.56.102 ./ring 10
